...
impl DhcpClient {
    ...
    pub fn get_state_tag(&self) -> &str {
        match self.state {
            DhcpState::Halted => "dhcpHalt",
            DhcpState::Init => "dhcpInit",
            DhcpState::Selecting => "dhcpSelect",
            DhcpState::Requesting => "dhcpRequest",
    ...
    pub fn cycle_clock(&mut self) -> PacketNeeded {
        ...
        match self.state {
            DhcpState::Halted => PacketNeeded::None,
            DhcpState::Init => {
                self.secs.start();
            ...
            DhcpState::Requesting => match self.retry.status() {
                RetryStatus::Halted => {
                    self.halt_and_reset();
                    PacketNeeded::None
                }
            ...
            DhcpState::Bound => match self.timer_t1.status() {
            ...
            DhcpState::Renewing => match self.timer_t2.status() {
                CountdownStatus::Done => {
                    self.timer_t2.clear();
