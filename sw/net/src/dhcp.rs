...
impl DhcpClient {
    ...
    pub fn cycle_clock(&mut self) -> PacketNeeded {
        ...
        match self.state {
            ...
            State::Selecting => {
                match self.ip {
                    ...
                    Some(_) => {
                        ...
                        // TODO: set exponential backoff timer for DISCOVER retry
                        PacketNeeded::Request
                    }
                    _ => {
                        // TODO: check elapsed time for exponential backoff on DISCOVER retry
                        let wait_for_offer_expired = true;
                        match wait_for_offer_expired {
                            true => {
                                // TODO: set exponential backoff timer for DISCOVER retry
            ...
            State::Requesting => {
                // TODO: check elapsed time for exponential backoff on REQUEST retry
                let wait_for_ack_expired = false;
                match wait_for_ack_expired {
                    true => {
                        // TODO: set exponential backoff timer for REQUEST retry
                        // TODO: notify main event loop of DHCP connection problem
                        PacketNeeded::Request
                    }
                    _ => PacketNeeded::None,
